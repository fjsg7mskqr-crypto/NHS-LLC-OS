...
import { MOCK_JOBS, MOCK_CLIENTS, MOCK_PROPERTIES, MOCK_TIME_ENTRIES } from '@/lib/mock-data'
...
export default function JobsList({ onSelect }: { onSelect: (id: string) => void }) {
  ...
  const enriched = MOCK_JOBS.map(job => {
    ...
    const billableHours = entries.filter(te => te.billable).reduce((s, te) => s + (te.duration_minutes || 0), 0) / 60
    const totalHours = entries.reduce((s, te) => s + (te.duration_minutes || 0), 0) / 60
    const billedAmount = entries.filter(te => te.billable).reduce((s, te) => s + ((te.duration_minutes || 0) * (te.rate || 0) / 60), 0)
    return { ...job, client, property, billableHours, totalHours, billedAmount }
  })
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-slate-800">
              ...
              <th className="text-right px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide hidden sm:table-cell">Billed</th>
            ...
            {filtered.map(job => (
              <tr key={job.id} onClick={() => onSelect(job.id)} className="hover:bg-slate-800/40 transition-colors cursor-pointer group">
                ...
                <td className="px-4 py-3.5 text-right">
                  <p className="text-sm font-mono text-slate-300">{formatHours(job.billableHours)}</p>
                  {job.totalHours > job.billableHours && <p className="text-xs text-slate-600">{formatHours(job.totalHours)} total</p>}
                </td>
                <td className="px-4 py-3.5 text-right hidden sm:table-cell"><span className="text-sm font-medium text-emerald-400">{job.billedAmount > 0 ? formatCurrency(job.billedAmount) : '—'}</span></td>
                <td className="px-4 py-3.5 text-right hidden sm:table-cell"><span className="text-sm text-slate-400">{job.target_rate ? `$${job.target_rate}/hr` : '—'}</span></td>
