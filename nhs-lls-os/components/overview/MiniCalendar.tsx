...
import { MOCK_CALENDAR_BLOCKS } from '@/lib/mock-data'
...
export default function MiniCalendar() {
  ...
  const blocks = MOCK_CALENDAR_BLOCKS.filter(b => b.start_date.startsWith(monthStr) || b.end_date.startsWith(monthStr))

  function getDayBlocks(day: number) {
    const dateStr = `${month.year}-${String(month.month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`
    return blocks.filter(b => b.start_date <= dateStr && b.end_date >= dateStr)
  }
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-7 gap-px">
          {cells.map((day, i) => {
            ...
            const hasSBR = dayBlocks.some(b => b.type === 'sbr_booking')
