...
import { getDailyTimelineData } from '@/lib/mock-data'
...
export default function DailyTimeline({ date }: { date: string }) {
  const entries = getDailyTimelineData(date)
          ...
          <>
              ...
              {entries.map(entry => {
                ...
                return (
                  <div key={entry.id} className="timeline-bar absolute top-1 bottom-1 rounded cursor-pointer"
                    ...
                    title={`${CATEGORY_LABELS[entry.category as CategoryType]} — ${formatMinutes(duration)}`}
                  />
                )
              })}
            </div>
            <div className="mt-4 space-y-2">
              {entries.map(entry => {
                ...
                return (
                  <div key={entry.id} className="flex items-center gap-3 p-3 rounded-lg bg-slate-800/40 hover:bg-slate-800/60 transition-colors">
