...
import { getWeeklyChartData } from '@/lib/mock-data'
      ...
      {[...payload].reverse().map((p: any) => (
        <div key={p.dataKey} className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-slate-400">{CATEGORY_LABELS[p.dataKey as keyof typeof CATEGORY_LABELS]}</span>
          </div>
          <span className="text-white font-mono">{p.value.toFixed(1)}h</span>
        </div>
      ))}
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900/50 overflow-hidden">
      <div className="px-5 py-4 border-b border-slate-800">
        ...
        <p className="text-xs text-slate-500 mt-0.5">Stacked by category, Mar 30 – Apr 5</p>
      </div>
      <div className="p-5">
